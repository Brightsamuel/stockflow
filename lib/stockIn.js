...
// Every line gets its own IN log, all sharing the receipt's ref no. and date.
export async function applyStockIn(tx, storeId, items, { refNo = null, entryDate, userId = null }) {
  const productIds = [...new Set(items.map(item => item.productId))]

  // Load current entries once; the map is kept up to date so the same product can appear on several lines
  const existing = await tx.stockEntry.findMany({ where: { storeId, productId: { in: productIds } } })
  ...
  for (const { productId, rate, quantity, lowStockAt } of items) {
    ...
    if (current?.isDeleted) {
      ...
      entry = await tx.stockEntry.create({
        data: { productId, storeId, rate, quantity, lowStockAt: lowStockAt ?? 0 },
      })
    } else if (current) {
  ...
  await tx.stockLog.createMany({
    data: items.map(({ productId, rate, quantity }) => ({
      storeId, productId, type: "IN", quantity, rate, userId, refNo, entryDate,
    })),
  })
}
