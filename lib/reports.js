...
async function balanceLogsFor(storeIds, before) {
  return prisma.stockLog.groupBy({
    ...
    where: { storeId: { in: storeIds }, entryDate: { lt: before } },
    _sum: { quantity: true },
  })
}
...
async function rangeLogsFor(storeIds, from, to) {
  return prisma.stockLog.groupBy({
    by: ["storeId", "productId", "type"],
    where: { storeId: { in: storeIds }, entryDate: { gte: from, lte: to } },
    _sum: { quantity: true },
  })
}
...
export async function earliestEntryDate(productId, storeId) {
  const first = await prisma.stockLog.findFirst({
    where: { productId, storeId, type: "IN" },
    orderBy: { entryDate: "asc" },
    select: { entryDate: true },
  })
  return first?.entryDate ?? null
}
